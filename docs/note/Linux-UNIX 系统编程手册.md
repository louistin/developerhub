...
* 数据传输工具
  * 读取和写入
  * 在用户内存和内核内存之间进行两次数据传输: `write()` `read()`
* 共享内存
  * 共享内存允许进程通过将数据放到由进程间共享的一块内存中以完成信息交换.
  * 通信无需系统调用以及用户内存与内核内存之间的数据传输

#### 数据传输

* 字节流
  * 管道, FIFO, 流式 socket
  * 交换数据是一个无分隔符的字节流, 每次可能读取任意数量的字节
* 消息
  * System V 消息队列, POSIX 消息队列, 数据报 socket
  * 以分隔符分隔的消息, 每个读取操作读取由写入者写入的一整条消息
* 伪终端
  * 特殊情况下的通信工具
...
* 三种形式
  ...
  * POSIX 共享内存
  * 内存映射
* 共享内存通信速度更快, 但需要同步. 一般使用信号量进行同步
* 放入共享内存中的数据对所有共享这块内存的进程可见

* 数据传输工具和共享内存差别
  * 数据传输工具读取具有破坏性, 读操作会消耗数据
  * 读取者和写入者之间的同步时原子的. 从空数据传输工具中读取数据默认会被阻塞直到写入数据

### 同步工具

* 信号量
  * 一个信号量是一个由内核维护的整数, 其值永远不会小于 0.
* 文件锁
  * 协调操作同一文件的多个进程动作
  * 读(共享)锁
  * 写(互斥)锁
...
* 功能
  * 数据传输工具提供读取和写入操作, 传输的数据只供一个读者消耗. 内核会自动处理读取和写入间的流
    控和同步
  * 其他应用程序设计使用共享内存方式. 一个进程通过共享内存能够使数据对共享同一区域的所有进程可
    见. 需要设计同步处理
  * 管道, FIFO, 流 socket 以字节流的形式传输数据, 消息队列和数据报 socket 面向消息
  * 消息队列能够给消息赋一个数值类型或优先级, 这样传送的消息可以和发送的消息顺序不同
  * 管道, FIFO, socket 是使用文件描述符实现的
  * POSIX 消息队列提供了一个通知工具, 当一条消息进入了一个之前为空的队列时可以使用它来向进程发
    送信号或实例化一个新线程
  * UNIX domain socket 提供一个特性允许在进程间传递文件描述符
  * UDP socket 允许一个发送者向多个接收者广播或组播一条消息
  * 使用 `fcntl()` 加上的记录锁由加锁的进程拥有, 获得记录锁的进程终止后会自动释放该记录锁
  * 只有 socket 允许网络通信
